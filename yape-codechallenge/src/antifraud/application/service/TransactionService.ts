...
export class TransactionAplicationService {
    constructor(private transactionDomainService: TransactionDomianService) { }

    async saveTransaction(transaction: TransactionRequest): Promise<TransactionResponse> {
        console.log("🚀 ~ TransactionAplicationService ~ saveTransaction ~ TransactionRequest:", transaction)
        try {
